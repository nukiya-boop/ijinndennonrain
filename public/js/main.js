(function () {
  ...
  function openRulebook() {
    ...
    quickStart.className = 'rb-quickstart';
    quickStart.innerHTML = `
      <div class="rb-quickstart-title">⚡ 3行でわかる遊び方</div>
      <ol>
        <li>自分の番に、手札のカードを使って<b>イジン(キャラクター)を戦場に出す</b>。</li>
        <li>自分のイジンで<b>アタック</b>し、相手が防げなければそのまま<b>勝利</b>!</li>
        <li>逆に相手にアタックされたら、自分の<b>イジンかガーディアン(裏向きカード)でブロック</b>して防ごう。</li>
      </ol>
      <p class="rb-quickstart-note">迷ったら、まずは「🤖 CPUと対戦する」で実際に触りながら覚えるのが一番早いです。細かいルールは下の項目を見てください。</p>
    `;
    wrap.appendChild(quickStart);
    ...
    wrap.appendChild(rulebookSection('🛡️ ⑥ ガーディアン', `
      <ul>
    ...
    wrap.appendChild(rulebookSection('✨ ⑦ よく出てくるキーワード能力・太文字能力・遺業能力', `
      <table>
      ...
      <div class="select-hint" style="text-align:left;margin-top:10px;">
        カードのテキストの先頭に太字で書かれている見出しは、発動するタイミングを表す
        「太文字能力」です。以下の5種類がよく登場します。
      </div>
      <table>
        <tr><th><span class="rb-kw">執筆</span></th><td>ハイケイが戦場に置かれたときに発動できる能力です(自分のハイケイが対象のことが多いですが、カードによって異なります)。</td></tr>
        <tr><th><span class="rb-kw">躍進</span></th><td>「このターンに魔力ゾーンの能力(遺業能力やマホウなどではなく、マリョク自体が持つ効果)によって山札からカードを引いている」場合にのみ効果を発揮する能力です。通常のドロー効果では条件を満たしません。</td></tr>
        <tr><th><span class="rb-kw">徴募</span></th><td>自分のエンドフェイズが開始したときに発動できる能力です。カードが戦場にある間だけでなく、墓地にある間もこの能力を発動できるのが特徴です。</td></tr>
        <tr><th><span class="rb-kw">決起</span></th><td>自分がマリョクを配置したときに発動できる能力です。</td></tr>
        <tr><th><span class="rb-kw">消耗</span></th><td>これが表向きで魔力ゾーンにある間、自分のスタートフェイズになると自動的に裏向きになります(効果は裏向きの間は発動しません)。再び表向きにするには、遺業能力の「復元」など、表にするカード固有の効果が必要です。</td></tr>
      </table>
      <div class="select-hint" style="text-align:left;margin-top:10px;">
        <b>遺業能力</b>は、イジン・ハイケイ・マホウ・マリョクが破壊されて墓地に置かれたときに
        「発動してもよい」任意の能力です(冥府発動・反魂を除く)。本アプリでは、破壊されると
        画面に「発動する/発動しない」を選ぶポップアップが表示されます。発動するかどうかは
        完全に自由で、発動しなくてもペナルティはありません。複数のカードが同時に破壊された
        場合は、破壊された順に1件ずつ解決します。
      </div>
    `))
